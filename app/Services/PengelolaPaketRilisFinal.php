...
class PengelolaPaketRilisFinal
{
    ...
    public function buat(string $versi, ?string $direktori = null): array
    {
        ...
        $this->pastikanRepositoryBersih($git);
        ...
        try {
            ...
            $this->simpanJson($direktoriSementara.'/manifest-rilis-final.json', $manifest);
            $this->simpanJson($direktoriSementara.'/inventaris-berkas.json', $inventaris);
            $this->salinCatatanRilis($direktoriSementara.'/RELEASE-NOTES.md');
            ...
            $this->buatDaftarChecksum($direktoriSementara, $komponen);
            $komponen[] = 'checksums.sha256';

            @unlink($lokasiPaket);
            @unlink($lokasiPaket.'.sha256');
            $this->jalankan([
                $tar,
                '-czf',
                $lokasiPaket,
    ...
    public function verifikasi(string $berkas): array
    {
        ...
        try {
            ...
            $manifest = $this->bacaJson($direktoriSementara.'/manifest-rilis-final.json');
            $inventaris = $this->bacaJson($direktoriSementara.'/inventaris-berkas.json');
            $pemeriksaan = [
                ...$this->verifikasiManifest($manifest),
                ...$this->verifikasiInventaris($tar, $direktoriSementara, $inventaris, $manifest),
            ];
    ...
    private function inventarisBerkasGit(string $git): array
    {
        ...
        foreach ($daftar as $berkas) {
            ...
            if (is_link($lokasi)) {
                throw new RuntimeException("Symlink {$berkas} tidak diizinkan pada paket rilis final.");
            }
    ...
    private function verifikasiInventaris(string $tar, string $direktori, array $inventaris, array $manifest): array
    {
        ...
        if ($arsip === '' || ! is_file($lokasiArsip) || ! is_array($berkasInventaris)) {
            return ['arsip_sumber' => false, 'inventaris_berkas' => false, 'checksum_inventaris' => false];
        }

        $daftar = $this->daftarArsip($tar, $lokasiArsip);
        $this->pastikanPathArsipAman($daftar);
        if (($inventaris['format'] ?? null) !== self::FORMAT_INVENTARIS) {
            return ['arsip_sumber' => true, 'inventaris_berkas' => false, 'checksum_inventaris' => false];
        }
        ...
        $daftarSama = $daftarNormal === $daftarInventaris
        ...
        $direktoriSumber = $direktori.'/sumber';
        File::ensureDirectoryExists($direktoriSumber, 0700, true);
        $this->jalankan([
            $tar,
            '-xzf',
            $lokasiArsip,
        ...
        $checksumValid = true;
        foreach ($berkasInventaris as $path => $metadata) {
            $lokasi = $direktoriSumber.DIRECTORY_SEPARATOR.$path;
            if (is_link($lokasi) || ! is_file($lokasi)) {
                $checksumValid = false;
                break;
            }
            $checksum = hash_file('sha256', $lokasi);
            if ($checksum === false
                || ! hash_equals((string) ($metadata['sha256'] ?? ''), $checksum)
                || (int) ($metadata['ukuran_byte'] ?? -1) !== (filesize($lokasi) ?: 0)) {
                $checksumValid = false;
                break;
            }
        }
        ...
        return [
            'arsip_sumber' => true,
            'inventaris_berkas' => true,
            'checksum_inventaris' => $checksumValid,
        ];
    }
    ...
    private function verifikasiManifest(array $manifest): array
    {
        $database = (string) config('database.connections.mysql.database');
        $versi = (string) ($manifest['versi'] ?? '');
        $baseTable = $this->jumlahObjekDatabase($database, 'BASE TABLE');
        $view = $this->jumlahObjekDatabase($database, 'VIEW');
        $permission = (int) DB::table('hak_akses')->whereNull('deleted_at')->count();
        $dilarang = $this->jumlahTabelDilarang($database);
        $batasan = $manifest['batasan'] ?? [];
        ...
        return [
            ...
            'versi_aplikasi' => trim((string) @file_get_contents(base_path('VERSION'))) === $versi
                && $versi === KontrakRilisFinal::VERSI,
            'base_table' => ($manifest['database']['base_table'] ?? null) === KontrakRilisFinal::JUMLAH_TABEL
                && $baseTable === KontrakRilisFinal::JUMLAH_TABEL,
            'view' => ($manifest['database']['view'] ?? null) === KontrakRilisFinal::JUMLAH_VIEW
                && $view === KontrakRilisFinal::JUMLAH_VIEW,
            'permission_aktif' => ($manifest['database']['permission_aktif'] ?? null) === KontrakRilisFinal::JUMLAH_PERMISSION
                && $permission === KontrakRilisFinal::JUMLAH_PERMISSION,
            'tabel_dilarang' => ($manifest['database']['tabel_infrastruktur_dilarang'] ?? null) === 0 && $dilarang === 0,
            'integritas_kritis' => $this->verifikasiChecksumKritis($manifest['integritas']['berkas_kritis'] ?? []),
            'batasan_keamanan' => is_array($batasan)
                && ($batasan['tanpa_kredensial'] ?? false) === true
                && ($batasan['tanpa_env_runtime'] ?? false) === true
                && ($batasan['tanpa_backup_database'] ?? false) === true
                && ($batasan['tanpa_log_runtime'] ?? false) === true
                && ($batasan['tanpa_data_transaksi'] ?? false) === true
                && ($batasan['deployment_otomatis'] ?? true) === false
                && ($batasan['auto_merge'] ?? true) === false,
        ];
    }
    ...
    {
        $aktual = $this->checksumBerkasKritis();
        if (! is_array($daftar) || array_keys($daftar) !== array_keys($aktual)) {
            return false;
        }
        foreach ($aktual as $berkas => $checksum) {
    ...
    private function buatDaftarChecksum(string $direktori, array $berkas): void
    {
        ...
        $lokasi = $direktori.'/checksums.sha256';
    ...
    {
        $baris = file($direktori.'/checksums.sha256', FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
    ...
    private function pastikanRepositoryBersih(string $git): void
    {
        $proses = new Process([$git, 'status', '--porcelain', '--untracked-files=no'], base_path());
        $proses->setTimeout(30);
        $proses->run();
        if (! $proses->isSuccessful()) {
            throw new RuntimeException('Status repository Git tidak dapat diperiksa.');
        }
        if (trim($proses->getOutput()) !== '') {
            throw new RuntimeException('Repository memiliki perubahan terlacak yang belum dikomit.');
        }
    }
